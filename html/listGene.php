...
$table = "fragments";
$columns = ['fragments.name', 'fragments.description', 'fragments.proteinSequence', 'fragments.resistance', 'fragments.origin', 'fragments.DNASequence', 'fragments.PCRoligo1', 'fragments.PCRoligo2', 'fragments.PCRtemplate', 'fragments.reaction', 'fragments.type', 'tracker.trackID', 'tracker.owner', 'tracker.permOwner'];
...
$fields = ['trackID' => 'ID', 'hexID' => 'Building Block', 'name' => 'Name', 'description' => 'Description', '1' => 'DNA Sequence', '2' => 'Protein'];

if($category == 'backbone'){
	$fields = ['trackID' => 'ID', 'hexID' => 'Building Block', 'name' => 'Name', 'description' => 'Description', 'resistance' => 'Resistance Marker', 'origin' => 'Origin of Replication', '3' => 'DNA Sequence'];
}

if($category == 'PCR'){
	$fields = ['trackID' => 'ID', 'hexID' => 'Building Block', 'name' => 'Name', 'PCRoligo1' => 'Oligo 1', 'PCRoligo2' => 'Oligo 2', '3' => "Tm1 / Tm2", 'PCRtemplate' => 'Template', '4' => 'Length'];
...
$categories = ['gene'=>'Genes/Proteins', 'PCR' => 'PCR Products', 'backbone' => 'Plasmid Backbones'];
#Set Menu items
?>
...
if($category == 'PCR'){
	...
	$oligos = [];
	$ocols = ['tracker.trackID', 'oligos.name', 'oligos.targetmatch', 'oligos.Saltconc', 'oligos.PCRconc', 'oligos.tm'];
	$olis = getRecords('oligos',$userid, [], $ocols);
	if($olis){
		foreach ($olis as $o) {
			$tm = $o['tm'] ? $o['tm']."(User)" : Tm($o['targetmatch'],'bre',$o['Saltconc']*1E-3, $o['PCRconc']*1E-9);
			$oligos[$o['trackID']] = ['name' => $o['name'], 'tm' => $tm];
		}
	}
	// get plasmids into array
	$plasmids = [];
	$pcols = ['tracker.trackID', 'plasmids.name'];
