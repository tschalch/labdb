...
function unigePrintEntry($entry){
    ...
    $priceCents = 100 * ($entry['price']-$priceInt);
    $unigeString .= "<td>$priceInt</td>";
    $unigeString .= "<td>$priceCents</td>";
    $total = $entry['quantity'] * $entry['price'];
    $orderTotal += $total;
    $totalInt = floor($total);
    $totalCents = 100 * ($total - $totalInt);
    $unigeString .= "<td>$totalInt</td>";
    $unigeString .= "<td>$totalCents</td>";
    $unigeString .= "</tr>";
    return $unigeString;
}
...
if ($_GET["output"]=='unige' and $_GET["selection"]){
    ...
    $orderTotalCents = 100 * ($orderTotal - $orderTotalInt);
